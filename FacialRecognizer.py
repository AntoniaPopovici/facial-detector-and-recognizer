...
class FacialRecognizer:
    ...
    def get_positive_descriptors_characters(self, path):
    ...
    def get_negative_descriptors_characters(self, path):
        ...
        for i, image_file in enumerate(files):
            print(
                f"Imagine procesata: {i + 1}/{len(files)}: {image_file}")
            image = cv2.imread(image_file)
            gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    def train_classifier(self, training_examples, train_labels, ignore_restore=True):
        svm_file_name = os.path.join(
            MODELS, 'model_recunoastere_faciala_barney')
